...
use libc::{c_char, c_int, c_long, c_ulonglong, c_void};
use core::mem::size_of;

// TODO: rewrite docs to be consistent with memapi's doc style
...
pub const MI_SMALL_SIZE_MAX: usize = 128 * size_of::<*mut c_void>();
...
extern "C" {
    ...
    /// equivalent testing against [`mi_usable_size`](mi_usable_size).
    ...
    /// initialization: [`mi_calloc`](mi_calloc),
    ...
    /// example.
    ///
    /// See [`mi_usable_size`](mi_usable_size).
...
/// Option (experimental) The number of huge OS pages (1GiB in size) to reserve at the start of the
/// program.
///
...
/// You can use `mi_option_reserve_huge_os_pages_at=N` where `N` is the numa node (starting at 0) to
/// allocate all
...
/// Option (experimental) Pretend there are at most N NUMA nodes; Use 0 to use the actual detected
/// NUMA nodes at runtime.
pub const mi_option_use_numa_nodes: mi_option_t = 16;

/// Option (experimental) If set to 1, do not use OS memory for allocation (but only pre-reserved
/// arenas)
...
extern "C" {
    ...
    /// `mi_option_` functions if needed, see the [mimalloc documentation](https://microsoft.github.io/mimalloc/group__options.html) for
    /// details.
    ///
    /// Note: this function is not thread safe.
